...
# No ClickUp credentials set? Appends to a local fallback file and exits 0 --
# this is expected for now, not an error. See DAILY-SUMMARY.md for the log.
...
if [[ -z "${CLICKUP_API_KEY:-}" || -z "${CLICKUP_WORKSPACE_ID:-}" || -z "${CLICKUP_CHANNEL_ID:-}" ]]; then
  printf "\n---\n## %s (fallback -- no notification channel configured)\n%s\n" "$stamp" "$msg" >> "$FALLBACK"
  echo "[notification fallback] appended to DAILY-SUMMARY.md"
  echo "$msg"
  exit 0
fi
