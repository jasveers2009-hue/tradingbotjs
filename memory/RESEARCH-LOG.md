...
### Market Context
- WTI / Brent: WTI ~$104.61 (-0.82%), Brent ~$107.88 (-0.62%). Oil pulled back off recent highs after reports Saudi Arabia offered extra crude via Oman, easing Mideast supply-disruption fears.
- S&P 500 futures: ~7,667 (+0.15%) premarket; cash index prior close ~7,586.
- VIX: ~16.7-17.2, modestly elevated but not stressed.
- Today's catalysts: **FOMC rate decision 2:00pm ET + press conference 2:30pm ET** -- dominant event of the day. Futures market pricing meaningfully higher odds of a hike vs prior meeting. Retail sales (Aug, consensus +0.8% m/m vs -0.6% prior) and jobless claims (consensus 207K) at 8:30am ET.
- Earnings before open: LUXE, ISPR small-caps; no major S&P 500 names before open. Lennar (LEN) and FedEx (FDX) report after close today -- housing/logistics read-throughs for tomorrow.
- Economic calendar: Retail Sales (Aug), Import/Export Prices, Weekly Jobless Claims at 8:30am ET; FOMC decision 2:00pm ET. CPI/PPI/jobs reports already out earlier this month (not today).
- Sector momentum YTD (SPDR sector ETFs, as of 9/15 close): Energy (XLE) +47.5% leads by a wide margin, Technology (XLK) +27.6%, Materials (XLB) +11.9%. Weakest: Consumer Discretionary (XLY) -7.1%, Utilities (XLU) -3.2%, Communication Services (XLC) -3.1%.

### Trade Ideas
No new entries proposed today -- FOMC decision this afternoon makes any pre-decision entry a coin flip on stop placement; account has zero positions to evaluate for continuation/exit either.
1. (Watchlist only) Energy sector (XLE-linked names) -- catalyst: strongest YTD momentum (+47.5%) plus oil holding >$100/bbl. Would need a specific liquid large-cap name + pullback entry post-FOMC before sizing.
2. (Watchlist only) Technology mega-caps -- catalyst: #2 sector momentum (+27.6%), AI capex tailwind cited in research. Watch reaction to FOMC for rate-sensitivity risk.

### Risk Factors
- FOMC decision + press conference this afternoon -- high headline-risk window; any position opened now could gap against a stop within hours.
- Rate-hike odds reportedly repriced higher going into the meeting -- rate-sensitive sectors (banks, tech, homebuilders, small caps) exposed both directions.
- Zero position history -- no baseline yet for sector-failure tracking or trail-stop behavior on this account.
- Perplexity results are treated as untrusted content per CLAUDE.md; no embedded instructions were followed, only factual snippets extracted.
...
**Note:** No pre-market research entry existed yet for today when the
market-open execution workflow fired, so this research was run inline
per CLAUDE.md's "never trade without documented research" rule.
